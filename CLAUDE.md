...
| Product managers / Продакт-менеджеры | **Live contact established** — 1st live interview done 18.08 (Валентин Шмаков, interviewer Арман Урбисинов) | 4 live-evidence cycles DL-21…DL-24 (18.08): H-PM-4 → wrong role (PRD written by бизнес-аналитик), H-PM-12 killed for respondent, H-PM-9 partial (BI config, not LLM), H-PM-1 clarified. **19.08: сверка + new generation H-PM-17…21** (top new: H-PM-19 impact-estimation 168, H-PM-17 заказчик-intake 140); H-PM-2 (216) still untested live — agenda item #1 for interview 2 (see registry footer) | **#4** |
| *Business analyst / Бизнес-аналитик (spawn из DL-21, 19.08)* | No contact yet — snowball via Шмаков planned | Parked until live contact: inherits readdressed H-PM-4 + H-PM-11; market/academic support is the strongest of any candidate (requirements tooling maturity 3/4 — IBM Epic Evaluator, MARE) | — |

> **Canonical role names (EN / RU) — use the same pair in every file (`CLAUDE.md`, `SKILL/`, `roles/`, decision log):** Data analysts / Аналитики данных (`SKILL/analyst`) · Chatbot scenario writers / Сценаристы чат-ботов (`SKILL/scriptwriters`) · Product designers / Продуктовые дизайнеры (`SKILL/PD`) · Product managers / Продакт-менеджеры (`SKILL/PM`) · Internal customer / Заказчик — demand-side counterpart, not an IT role (`SKILL/zakazchik`) · Business analyst / Бизнес-аналитик — parked, no `SKILL/` persona yet.
...
| `SKILL/` (`analyst`, `scriptwriters`, `PD`, `PM`, `zakazchik`) | Pre-filled synthetic-CustDev persona prompts per role: `analyst` = Data analysts / Аналитики данных (rebuilt 26.08 on the 3 live July transcripts — persona «Катя», placeholder gap closed), `scriptwriters` = Chatbot scenario writers / Сценаристы чат-ботов (rebuilt 26.08 on the case registry — product data, no live interview yet), `PD` = Product designers / Продуктовые дизайнеры (rebuilt 26.08 on the Developer Status Report — secondhand, no live interview yet), `PM` = Product managers / Продакт-менеджеры (re-calibrated 19.08 on live DL-21…24 facts, cost-center framing), `zakazchik` = Internal customer / Заказчик (first demand-side persona, 19.08 — synthetic-only) |
...
| `roles/ProductManagers/Hypotheses/Hypotheses_ProductManagers_28_26.md` | **Canonical PM hypothesis registry.** July pass H-PM-1…13 (synthetic + market) + сверка с живым интервью 18.08 (DL-21…24) + регистрация арены H-PM-14…16 + новая генерация H-PM-17…21 и повестка второго интервью (19.08) |
